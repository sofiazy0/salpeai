...
import { apiError, json, quota, requireUser } from "@/lib/server";
...
export async function DELETE(request: Request, context: Context) {
  try {
    ...
    const id = uuidSchema.parse((await context.params).id);
    const { user, client } = await requireUser();
    await quota("delete", user.id, 30, 60);
    const { data, error } = await client.from("salpe_conversations").delete().eq("id", id).eq("user_id", user.id).select("id");
    if (error) throw error;
    if (!data.length) throw new AppError(404, "Conversa não encontrada.");
    return json({ ok: true });
  } catch (e) { return apiError(e); }
}
